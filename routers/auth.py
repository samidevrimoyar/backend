from fastapi import APIRouter, Depends, HTTPException, status # 'Form'u kaldırdık
...
# tokenUrl'i kaldırdık. Bu, Swagger UI'da doğrudan token yapıştırma alanını sağlar.
oauth2_scheme = OAuth2PasswordBearer(tokenUrl=None) # Veya sadece OAuth2PasswordBearer() de yazabilirsiniz
...
# Giriş isteği için Pydantic modeli - Tekrar etkinleştirildi
...
# Giriş endpoint'i - Tekrar JSON body kabul edecek şekilde GÜNCELLENDİ
@router.post("/login", summary="Authenticate user and get JWT token")
def login(request: LoginRequest, db: Session = Depends(get_db)): # request: LoginRequest olarak geri döndük
    ...
    return {
        ...
        "access_token": access_token, # Burası "access_token" olarak kalmalı
        "token_type": "bearer",
